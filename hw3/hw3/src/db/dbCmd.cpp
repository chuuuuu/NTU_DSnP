...
DBAppendCmd::exec(const string& option)
{
   ...
   if(!dbtbl)
   {
      cout<<"Error: Table is not yet created!!"<<endl;
   ...
   if(!myStrNCmp(row,tokens[0],2))
   {
      ...
      for(size_t i=0; i<dbtbl.nCols(); i++)
      {
         ...
         {
            int tmp;
            if(tokens[1+i]=="-") r.addData(INT_MAX);
            else if(myStr2Int(tokens[1+i], tmp))
               r.addData(tmp);
            else 
            {
               cout<<"Error: “"<<tokens[1+i]<<"” is not an integer!!"<<endl;
               return CMD_EXEC_ERROR;
            }
         }
   ...
   else if(!myStrNCmp(col,tokens[0],2))
   {
      vector<int> ivec;
      for(size_t i=0; i<dbtbl.nRows(); i++)
      {
         ...
         {
            int tmp;
            if(tokens[1+i]=="-") ivec.push_back(INT_MAX);
            else if(myStr2Int(tokens[1+i], tmp))
               ivec.push_back(tmp);
            else
            {
...
{  
   ...
   int row_idx, col_idx;
   lexOptions(option, tokens, 0);
   string row = "-Row", col = "-Column", t = "-Table", s = "-Summary";
   if(tokens.size()==0) return CmdExec::errorOption(CMD_OPT_MISSING, "");
   else if(!myStrNCmp(row,tokens[0],2))
   {
      if(tokens.size()==1) return CmdExec::errorOption(CMD_OPT_MISSING, tokens[0]);
      else if(tokens.size()>2) return CmdExec::errorOption(CMD_OPT_EXTRA, tokens[2]);
      else if(checkRowIdx(tokens[1],row_idx)) cout<<dbtbl[row_idx];
   }
   else if(!myStrNCmp(col,tokens[0],2))
   {
      if(tokens.size()==1) return CmdExec::errorOption(CMD_OPT_MISSING, tokens[0]);
      else if(tokens.size()>2) return CmdExec::errorOption(CMD_OPT_EXTRA, tokens[2]);
      else if(checkColIdx(tokens[1],col_idx)) dbtbl.printCol(col_idx);
   }
   else if(!myStrNCmp(t,tokens[0],2))
   {
      if(tokens.size()>2) return CmdExec::errorOption(CMD_OPT_EXTRA, tokens[2]);
      else  cout<<dbtbl;
   }
   else if(!myStrNCmp(s,tokens[0],2))
   {
      if(tokens.size()>2) return CmdExec::errorOption(CMD_OPT_EXTRA, tokens[2]);
   ...
   {
      if(checkRowIdx(tokens[0],row_idx)) return CmdExec::errorOption(CMD_OPT_MISSING, tokens[0]);
   }
   else
   {
     if(checkRowIdx(tokens[0],row_idx))
         if(checkColIdx(tokens[1],col_idx))
            if(tokens.size()>2) return CmdExec::errorOption(CMD_OPT_EXTRA, tokens[2]);
            else if(dbtbl[row_idx][col_idx]==INT_MAX) cout<<"null"<<endl;
            else cout<<dbtbl[row_idx][col_idx]<<endl;
   }
   return CMD_EXEC_DONE;
}
