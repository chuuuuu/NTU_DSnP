...
#define downtoSizeT(t)  t-t%SIZE_T  // TODO
        ...
        class MemRecycleList
        {
           ...
           T* popFront() {
              ...
              {
                 T* tmp = this->_first;
                 this->_first = getNext(this->_first);
                 return tmp;
              }
           }
           ...
           void reset() {
              // TODO
              MemRecycleList* ptr=this->_nextList;
              while(ptr)
              {
                 MemRecycleList<T>* tmp = ptr->_nextList;
                 ptr->_nextList = NULL;
                 delete ptr;
                 ptr = tmp;
              }
    ...
    for(size_t i=0; i<R_SIZE; i++)
	 ...
	 //_recycleList[i]._arrSize = i;
              ...
              MemBlock<T>* tmp1 = this->_activeBlock;
              while(tmp1->_nextBlock!=NULL)
              {
                 MemBlock<T>* tmp2 = tmp1->_nextBlock;
                 delete tmp1;
                 tmp1 = tmp2;
              }
              this->_activeBlock = tmp1;

              if(b != 0)
              {
                 this->_blockSize = b;
      }
	 this->_activeBlock = new MemBlock<T>(0, _blockSize);
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      if(tmp->_arrSize == n) return tmp;
      else
      {
         while(tmp->_nextList)
         {
            if(n == tmp->_nextList->_arrSize) break;
	    else tmp = tmp->_nextList;
         }
         if(tmp->_nextList == NULL)
         {
            MemRecycleList<T>* tmp1 = new MemRecycleList<T>(n);
            tmp -> _nextList = tmp1;
            tmp1 -> _arrSize = n;
         }
         return tmp->_nextList;
      }
   ...
   T* getMem(size_t t) {
      ...
      {
         //cout<<(t == toSizeT(S));
         MemRecycleList<T>* aa = getMemRecycleList(0);
         if(aa->numElm() != 0) 
            return aa->popFront();
         else
         {
            if(this->_activeBlock->getRemainSize()<t)
            {
               this->_activeBlock = new MemBlock<T>(this->_activeBlock, this->_blockSize);
            }
      ...
      {
         size_t m = (t - SIZE_T)/S;
         MemRecycleList<T>* aa = getMemRecycleList(m);
               if(aa->numElm() != 0)
               {
                  //cout<<"111111111111";
                  return aa->popFront();
         ...
         {
            if((_activeBlock->getRemainSize())>=(toSizeT(S)+SIZE_T))
            {//recycle it
               //cout<<"333333333333";
               size_t arrSize = ((_activeBlock->getRemainSize()-SIZE_T)/S);
               getMemRecycleList(arrSize) -> pushFront((T*)(_activeBlock->_ptr));
            }
	    else if((_activeBlock->getRemainSize())>=toSizeT(S))
{
